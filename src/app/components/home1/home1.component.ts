...
@Component({
  ...
  styleUrls: ['./home1.component.css', './menu.css']
})
export class Home1Component implements OnInit {
  	...
  	$.getScript("../../../assets/js/scripts.js", function() {
		...
		this.mS.showcase().subscribe((r:any)=>{
			...
			for (var i = 0; i < r.length/2; i++){
				this.showcase1.push(r[i]);
			}
			for (var i = r.length/2+1; i < r.length; i++){
				this.showcase2.push(r[i]);
			}
			console.log(r);
		})
  ...
  openTry(): void {
    let dialogRef = this.dialog.open(TryNow, {
      width: 'auto',
      height: 'auto',
      data: { name: "Vishal Pandey", animal: "fkjdshk", showcase: this.showcase }
    });
